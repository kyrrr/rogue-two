...
public class Actor implements ActorInterface, Printable {
    ...
    public void move(CharKey dir) {
        if(alive){
            ...
            if(status.isPoisoned()){
                status.setHealth(status.getHealth() - status.getPoisonStack());
            }
        }
    }
    ...
    public void doCombatTurnStatuses(){}

    public void attackSelf(Move move){
        if(alive){
            move.expend();
            if(status.isPoisoned()){
                status.setHealth(status.getHealth() - status.getPoisonStack());
    ...
    public void attack(Actor actor, Move move) {
        if(alive){
            //todo: if move expendable
            move.expend();
            if(status.isPoisoned()){
                status.setHealth(status.getHealth() - status.getPoisonStack());
            } //todo: statuses
            if(!actor.getStatus().isProtected()){
    ...
    public void handleMove(Move move) {
        //move.getFirstEffect().affect(this); //todo:merge
       // if (!move.getEffects().isEmpty()){
    ...
    @Override
    public int getCharCodeMoveTowards(Actor actor) {
    ...
    @Override
    public void setDeathModel(String deathModel) {
        this.deathModel = deathModel;
    }

    @Override
    public String getDeathModel() {
        return deathModel;
    }
